...
namespace OxfordOnline.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<IEnumerable<ProductComplete>> GetAppProductAsync(string? product)
        {
            ...
            //var bom             = await _context.ProductBom.Where(t => t.ProductId == productId).ToListAsync();
            var bom = await (
                from b in _context.ProductBom
                where b.ProductId == productId
                join p in _context.Product on b.ProductBomId equals p.ProductId into pj
                from p in pj.DefaultIfEmpty()
                select new ProductBom
                {
                    Id = b.Id,
                    ProductId = b.ProductId,
                    ProductBomId = b.ProductBomId,
                    ProductName = string.IsNullOrEmpty(b.ProductName) ? p.ProductName : b.ProductName,
